...
int CountUsedTiles(const unsigned char* map,const unsigned char* mega,int* map2tile,int mapl)
{
	...
	for( j=0; j<0x10; ++j ) {
	    ...
	    printf("%04X " _C_ AccessLE16(tp+j*2));
...
#endif
    numtiles=msize/32;
    printf("Tiles in mega %d\t" _C_ numtiles);
	...
	,unsigned char* image,int ix,int iy,int iadd)
{
    ...
    for( h=0; h<height; ++h ) {
	printf("%2d: row-offset %2d\t" _C_ index _C_ (AccessLE16(rows+h*2)++));
	...
	,unsigned char* bp2,int start2)
{
    ...
    printf("Entries %2d Max width %3d height %3d, " _C_ count
...
unsigned char* ConvertFnt(unsigned char* start,int *wp,int *hp)
{
	...
	for( ;; ) {
	    ...
	    ctrl=FetchByte(bp);
	    printf("%d,%d " _C_ ctrl>>3 _C_ ctrl&7);
	    w+=(ctrl>>3)&0x1F;
	    if( w>=width ) {
		printf("\n");
		w-=width;
